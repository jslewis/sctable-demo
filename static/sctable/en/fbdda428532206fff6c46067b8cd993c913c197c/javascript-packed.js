...
if(!this[b]){this[b]=[]}return this[b]},createItemViewFromExampleView:function(c,b){var a=this.createItemView(c,null,b);
...
}};SC.CollectionGroup={classNames:["sc-collection-group"]};SC.CollectionRowDelegate={isCollectionRowDelegate:YES,itemHeight:18,rowSpacing:0,rowPadding:0,rowHeight:function(b,c){var a=this.get("rowPadding");
var d=this.get("itemHeight");if(c!==undefined){this.set("itemHeight",c-a*2);return c
}return d+a*2}.property("itemHeight","rowPadding"),customRowHeightIndexes:null,contentIndexRowHeight:function(a,b,c){return this.get("rowHeight")
...
if(isNaN(c)){return undefined}return parseInt(c,0)}});SC.BaseTheme.segmentRenderDelegate=SC.Object.create({render:function(e,b){var c=e.get("theme"),d,a;
a={"sc-first-segment":e.get("isFirstSegment"),"sc-middle-segment":e.get("isMiddleSegment"),"sc-last-segment":e.get("isLastSegment"),"sc-overflow-segment":e.get("isOverflowSegment")};
if(!SC.none(e.get("index"))){a["sc-segment-"+e.get("index")]=YES}b.setClass(a);d=c.buttonRenderDelegate;
d.render(e,b)},update:function(f,a){var d=f.get("theme"),e,c,b={};b={"sc-first-segment":f.get("isFirstSegment"),"sc-middle-segment":f.get("isMiddleSegment"),"sc-last-segment":f.get("isLastSegment"),"sc-overflow-segment":f.get("isOverflowSegment")||NO};
if(!SC.none(f.get("index"))){b["sc-segment-"+f.get("index")]=YES}a.setClass(b);e=d.buttonRenderDelegate;
e.update(f,a)}});SC.BaseTheme.segmentedRenderDelegate=SC.Object.create({render:function(b,a){a.addStyle("text-align",b.get("align"))
},update:function(b,a){a.css("text-align",b.get("align"))},segmentWidths:function(f){var e=f.$(".sc-segment-view"),b,d=[];
for(var a=0,c=e.length;a<c-1;a++){b=e[a];d[a]=b.getBoundingClientRect().width}return d
},overflowSegmentWidth:function(c){var b=c.$(".sc-segment-view"),a;a=b[b.length-1];
return a.getBoundingClientRect().width},indexForClientPosition:function(a,h,f){var e=a.$(".sc-segment-view"),b,d,c,g,j;
j={x:h,y:f};for(d=0,b=e.length;d<b;d++){c=e[d];g=c.getBoundingClientRect();g.x=g.left;
g.y=g.top;if(SC.pointInRect(j,g)){return d}}return -1}});SC.BaseTheme.sliderRenderDelegate=SC.RenderDelegate.create({name:"slider",render:function(c,b){var a=SC.BLANK_IMAGE_URL;
...
SC.DRAG_AUTOSCROLL_ZONE_THICKNESS=20;SC.View.reopen({init:function(a){a();if(this.get("isDropTarget")){SC.Drag.addDropTarget(this)
}if(this.get("isScrollable")){SC.Drag.addScrollableView(this)}}.enhance(),destroy:function(a){if(this.get("isDropTarget")){SC.Drag.removeDropTarget(this)
}if(this.get("isScrollable")){SC.Drag.removeScrollableView(this)}return a()}.enhance()});
SC.Drag=SC.Object.extend({source:null,ghostView:null,ghostActsLikeCursor:NO,dragView:null,ghost:YES,sourceIsDraggable:YES,slideBack:YES,ghostOffset:{x:0,y:0},location:{},dataTypes:function(){if(this.dataSource){return this.dataSource.get("dragDataTypes")||[]
...
this._lastHoveredItem=null;if(a&&a.mouseExited){a.mouseExited(b)}return YES},touchStart:function(c,a){this.becomeFirstResponder();
if(!this.get("useToggleSelection")){var b=this.itemViewForEvent(c);if(b&&!b.get("isSelected")){b.set("isSelected",YES);
this._touchSelectedView=b}else{this._touchSelectedView=null}}return YES},touchesDragged:function(a,b){b.forEach(function(c){if(Math.abs(c.pageX-c.startX)>5||Math.abs(c.pageY-c.startY)>5){c.makeTouchResponder(c.nextTouchResponder)
}},this)},touchEnd:function(e){var b=this.itemViewForEvent(e),d=b?b.get("contentIndex"):-1,a=NO;
if(this._touchSelectedView){this._touchSelectedView.set("isSelected",NO)}if(this.get("useToggleSelection")){var c=this.get("selection");
a=c&&c.containsObject(b.get("content"))}if(a){this.deselect(d)}else{this.select(d,NO);
this._cv_performSelectAction(b,e,0)}},touchCancelled:function(a){if(this._touchSelectedView){this._touchSelectedView.set("isSelected",NO)
...
SC.ANCHOR_RIGHT={right:0};SC.ANCHOR_BOTTOM_RIGHT={bottom:0,right:0};SC.ToolbarView=SC.View.extend({classNames:["sc-toolbar-view"],anchorLocation:null,layout:{left:0,height:32,right:0},init:function(){if(this.anchorLocation){this.layout=SC.merge(this.layout,this.anchorLocation)
...
}e.adjust(m)},scrollerVisibilityDidChange:function(){this.tile()}.observes("isVerticalScrollerVisible","isHorizontalScrollerVisible"),_scroll_wheelDeltaX:0,_scroll_wheelDeltaY:0,mouseWheel:function(a){var b=(SC.browser.webkit&&SC.browser.version>533)?120:1;
...
var d=this.get("overflowTitle"),a=this.get("overflowToolTip"),b=this.get("overflowIcon"),c;
c=SC.SegmentView.create({controlSize:this.get("controlSize"),localize:this.get("localize"),title:d,toolTip:a,icon:b,isLastSegment:YES,isOverflowSegment:YES});
this.appendChild(c);this.itemsDidChange()},itemsDidChange:function(){var k=this.get("items")||[],p,c,m=this.get("childViews"),h,d=m.lastObject(),n=this.get("value"),b,o=this.get("itemKeys"),a,l=this.get("viewKeys"),f,g,e;
if(m.get("length")-1>k.get("length")){for(g=m.get("length")-2;g>=k.get("length");
g--){h=m.objectAt(g);if(SC.isArray(n)){n.removeObject(h.get("value"))}else{if(n===h.get("value")){n=null
}}this.removeChild(h)}this.set("value",n)}else{if(m.get("length")-1<k.get("length")){for(g=m.get("length")-1;
g<k.get("length");g++){h=SC.SegmentView.create({controlSize:this.get("controlSize"),localize:this.get("localize")});
this.insertBefore(h,d)}}}m=this.get("childViews");for(g=0;g<k.get("length");g++){c=k.objectAt(g);
h=m.objectAt(g);if(SC.none(c)){continue}if(SC.typeOf(c)===SC.T_STRING){c=SC.Object.create({title:c.humanize().titleize(),value:c});
this.set("itemTitleKey","title");this.set("itemValueKey","value")}else{if(SC.typeOf(c)===SC.T_HASH){c=SC.Object.create(c)
}else{if(c instanceof SC.Object){for(e=o.get("length")-1;e>=0;e--){a=this.get(o.objectAt(e));
if(a){c.removeObserver(a,this,this.itemContentDidChange);c.addObserver(a,this,this.itemContentDidChange,g)
}}}else{SC.Logger.error("SC.SegmentedView items may be Strings, Objects (ie. Hashes) or SC.Objects only")
}}}b=NO;if(SC.isArray(n)?n.indexOf(c.get(this.get("itemValueKey")))>=0:n===c.get(this.get("itemValueKey"))){b=YES
}h.set("isSelected",b);h.set("index",g);h.set("isFirstSegment",g===0);h.set("isMiddleSegment",g<k.get("length")-1&&g>0);
h.set("isLastSegment",g===k.get("length")-1);h.updateItem(this,c)}this.invokeLast(this.remeasure)
}.observes("*items.[]"),itemContentDidChange:function(e,d,b,c){var a=this.get("items"),f=this.get("childViews"),g;
...
}this.invokeLast(this.remeasure)},viewDidResize:function(){var a=this.$().width();
if(this.isOverflowing||a<=this.cachedMinimumWidth){this.adjustOverflow()}},isVisibleInWindowDidChange:function(){this.invokeLast(this.remeasure)
}.observes("isVisibleInWindow"),remeasure:function(){var a=this.get("renderDelegate"),d=this.get("childViews"),c;
if(this.get("isVisibleInWindow")){c=d.lastObject();c.set("isVisible",YES);for(var b=d.get("length")-1;
b>=0;b--){d.objectAt(b).set("isVisible",YES)}this.cachedWidths=a.segmentWidths(this);
this.cachedOverflowWidth=a.overflowSegmentWidth(this);this.adjustOverflow()}},adjustOverflow:function(){var h=this.get("childViews"),f,j=this.get("value"),b=h.lastObject(),e=this.$().width(),d=0,g,a,c;
this.isOverflowing=NO;b.set("isSelected",NO);this.overflowItems=[];a=this.cachedWidths.length;
for(c=0;c<a;c++){f=h.objectAt(c);d+=this.cachedWidths[c];g=(c===a-1)?d:d+this.cachedOverflowWidth;
if(g>e){this.overflowItems.pushObject(f.get("localItem"));this.isOverflowing=YES;
f.set("isVisible",NO);if(c===0){b.set("isFirstSegment",YES)}if(SC.isArray(j)?j.indexOf(f.get("value"))>=0:j===f.get("value")){b.set("isSelected",YES)
}}else{f.set("isVisible",YES);if(c===0){b.set("isFirstSegment",NO)}}}if(this.isOverflowing){b.set("isVisible",YES)
}else{b.set("isVisible",NO)}this.cachedMinimumWidth=d+this.cachedOverflowWidth},displayProperties:["align"],renderDelegateName:"segmentedRenderDelegate",displayItemIndexForEvent:function(a){var b=this.get("renderDelegate");
if(b&&b.indexForClientPosition){return b.indexForClientPosition(this,a.clientX,a.clientY)
}},keyDown:function(c){var g,h,d,e,f,b;if(c.which===9||c.keyCode===9){var a=c.shiftKey?this.get("previousValidKeyView"):this.get("nextValidKeyView");
if(a){a.becomeFirstResponder()}else{c.allowDefault()}return YES}if(!this.get("allowsMultipleSelection")){g=this.get("childViews");
e=g.get("length");f=this.get("value");b=SC.isArray(f);if(c.which===39||c.which===40){if(f){for(d=0;
d<e-2;d++){h=g.objectAt(d);if(b?(f.indexOf(h.get("value"))>=0):(h.get("value")===f)){this.triggerItemAtIndex(d+1)
}}}else{this.triggerItemAtIndex(0)}return YES}else{if(c.which===37||c.which===38){if(f){for(d=1;
d<e-1;d++){h=g.objectAt(d);if(b?(f.indexOf(h.get("value"))>=0):(h.get("value")===f)){this.triggerItemAtIndex(d-1)
}}}else{this.triggerItemAtIndex(e-2)}return YES}}}return NO},mouseDown:function(a){var c=this.get("childViews"),e,d=c.get("length")-1,b;
if(!this.get("isEnabled")){return YES}b=this.displayItemIndexForEvent(a);if(b>=0){e=c.objectAt(b);
e.set("isActive",YES);this.activeChildView=e;if(b===d){this.showOverflowMenu()}else{this._isMouseDown=YES
}}return YES},mouseUp:function(a){var c,b;b=this.displayItemIndexForEvent(a);if(this._isMouseDown&&(b>=0)){this.triggerItemAtIndex(b);
c=this.activeChildView;c.set("isActive",NO);this.activeChildView=null;this._isMouseDown=NO
}return YES},mouseMoved:function(a){var d=this.get("childViews"),e=d.get("length")-1,c,f,b;
if(this._isMouseDown){b=this.displayItemIndexForEvent(a);c=this.activeChildView;f=d.objectAt(b);
if(f&&f!==c){if(c){c.set("isActive",NO)}f.set("isActive",YES);this.activeChildView=f;
if(b===e){this.showOverflowMenu();this._isMouseDown=NO}}}return YES},mouseEntered:function(a){var c=this.get("childViews"),e,d=c.get("length")-1,b;
if(this._isMouseDown){b=this.displayItemIndexForEvent(a);if(b===d){this.showOverflowMenu();
this._isMouseDown=NO}else{if(b>=0){e=c.objectAt(b);e.set("isActive",YES);this.activeChildView=e
}}}return YES},mouseExited:function(a){var b;if(this._isMouseDown){b=this.activeChildView;
if(b){b.set("isActive",NO)}this.activeChildView=null}return YES},touchStart:function(e){var b=this.get("childViews"),d,c=b.get("length")-1,a;
if(!this.get("isEnabled")){return YES}a=this.displayItemIndexForEvent(e);if(a>=0){d=b.objectAt(a);
d.set("isActive",YES);this.activeChildView=d;if(a===c){this.showOverflowMenu()}else{this._isTouching=YES
}}return YES},touchEnd:function(c){var b,a;a=this.displayItemIndexForEvent(c);if(this._isTouching&&(a>=0)){this.triggerItemAtIndex(a);
b=this.activeChildView;b.set("isActive",NO);this.activeChildView=null;this._isTouching=NO
}return YES},touchesDragged:function(a,e){var h=this.touchIsInBoundary(a),d=this.get("childViews"),f=d.get("length")-1,c,g,b;
if(h){if(!this._isTouching){this._touchDidEnter(a)}b=this.displayItemIndexForEvent(a);
c=this.activeChildView;g=d[b];if(g&&g!==c){if(c){c.set("isActive",NO)}g.set("isActive",YES);
this.activeChildView=g;if(b===f){this.showOverflowMenu();this._isMouseDown=NO}}}else{if(this._isTouching){this._touchDidExit(a)
}}this._isTouching=h;return YES},_touchDidExit:function(a){var b;if(this.isTouching){b=this.activeChildView;
b.set("isActive",NO);this.activeChildView=null}return YES},_touchDidEnter:function(a){var c=this.get("childViews"),e,d=c.get("length")-1,b;
b=this.displayItemIndexForEvent(a);if(b===d){this.showOverflowMenu();this._isTouching=NO
}else{if(b>=0){e=c.objectAt(b);e.set("isActive",YES);this.activeChildView=e}}return YES
},triggerItemAtIndex:function(i){var k=this.get("childViews"),g,b,l,c,h,f;g=k.objectAt(i);
if(!g.get("isEnabled")){return this}h=this.get("allowsEmptySelection");f=this.get("allowsMultipleSelection");
...
if(n&&(m=this.get("items").objectAt(i))){e=m.get?m.get(n):m[n];if(a){j=m.get?m.get(a):m[a]
...
}},selectOverflowItem:function(b){var a=b.get("selectedItem");this.triggerItemAtIndex(a.get("index"));
b.removeObserver("selectedItem",this,"selectOverflowItem");this.activeChildView.set("isActive",NO);
this.activeChildView=null},showOverflowMenu:function(){var f=this.get("childViews"),b=f.get("length")-1,o=this.overflowItems,k,j,e,h;
k=o.get("length");j=f.get("length")-1-k;h=this.get("value");e=SC.isArray(h);for(var c=0;
c<k;c++){var n=o.objectAt(c),l=this.get("itemValueKey");if(e?h.indexOf(n.get(l))>=0:h===n.get(l)){n.set("isChecked",YES)
}else{n.set("isChecked",NO)}n.set("index",j+c)}var m=this;var a=SC.MenuPane.create({layout:{width:200},items:o,itemTitleKey:this.get("itemTitleKey"),itemIconKey:this.get("itemIconKey"),itemIsEnabledKey:this.get("itemIsEnabledKey"),itemKeyEquivalentKey:this.get("itemKeyEquivalentKey"),itemCheckboxKey:"isChecked",modalPaneDidClick:function(){arguments.callee.base.apply(this,arguments);
this.removeObserver("selectedItem",m,"selectOverflowItem");m.activeChildView.set("isActive",NO);
m.activeChildView=null}});var d=this.get("layer");var g=d.childNodes[d.childNodes.length-1];
a.popup(g);a.addObserver("selectedItem",this,"selectOverflowItem")},valueDidChange:function(){var d=this.get("value"),c,f=this.get("childViews"),g=Infinity,e=f.lastObject(),h,a;
if(this.overflowItems){c=this.overflowItems.get("length");g=f.get("length")-1-c;e.set("isSelected",NO)
}for(var b=f.get("length")-2;b>=0;b--){h=f.objectAt(b);if(SC.isArray(d)?d.indexOf(h.get("value"))>=0:d===h.get("value")){h.set("isSelected",YES);
if(b>=g){e.set("isSelected",YES)}}else{h.set("isSelected",NO)}}}.observes("value"),acceptsFirstResponder:function(){if(!SC.SAFARI_FOCUS_BEHAVIOR){return this.get("isEnabled")
...
j=0;k.forEach(function(w){if(w||w===0){v=r?(w.get?w.get(r):w[r]):w.toString();v=n?v.loc():v;
...
SC.Event.remove(a,"blur",this,this.fieldDidBlur)}});SC.SliderView=SC.View.extend(SC.Control,{classNames:"sc-slider-view",value:0.5,valueBindingDefault:SC.Binding.single().notEmpty(),minimum:0,minimumBindingDefault:SC.Binding.single().notEmpty(),contentMinimumKey:null,maximum:1,maximumBindingDefault:SC.Binding.single().notEmpty(),contentMaximumKey:null,step:0.1,displayProperties:"displayValue minimum maximum step frame".w(),renderDelegateName:"sliderRenderDelegate",displayValue:function(){var b=this.get("minimum"),a=this.get("maximum"),d=this.get("value"),c=this.get("step");
...
}this._insertionPointView=null},beginColumnResizeDrag:function(){},updateColumnResizeDrag:function(a,c,b,d){if(this.get("shouldLiveResize")){this._resizeData={evt:a,col:c,colIndex:b,newWidth:d};
this.invokeOnce("_overrideColumnWidth")}},endColumnResizeDrag:function(){this._resizeData=null
},_sortDidChange:function(){this.invokeOnce("_updateSortView")}.observes("sort"),_updateSortView:function(){var f=this.get("childViews"),d,b;
var c=this.get("sort");var e=c?c.valueKey:null;var a=c?c.direction:null;if(f){for(d=0;
d<f.length;d++){b=f[d].get("content");if(b){if(b.get("valueKey")===e){f[d].set("sortDirection",a)
}else{f[d].set("sortDirection",null)}}}}},_overrideColumnWidth:function(){var b,d,a,c,e,f;
if(this._resizeData){b=this.get("content");d=b?b.get("length"):0;this.beginPropertyChanges();
if(this._resizeData.colIndex<d){c=this.itemViewForContentIndex(this._resizeData.colIndex);
...
}if(d){b=b.addClass(d)}b=b.begin("div").addClass("col-name").text(this.getPath("content.name")).end();
...
if(b.target.className==="resize-handle"){this._isDraggingHandle=YES;this._mouseDownInfo={didNotifyBeginResize:NO,didMove:NO,startPageX:b.pageX,startWidth:this.get("frame").width};
c=YES}return c},mouseDragged:function(b){var a,c;if(this._isDraggingHandle){this._mouseDownInfo.didMove=YES;
a=this.get("displayDelegate");if(a&&a.isTableColumnsDelegate){if(!this._mouseDownInfo.didNotifyBeginResize){a.beginColumnResizeDrag();
this._mouseDownInfo.didNotifyBeginResize=YES}else{c=Math.max(this._mouseDownInfo.startWidth+b.pageX-this._mouseDownInfo.startPageX,this.get("minWidth"));
a.updateColumnResizeDrag(b,this.get("content"),this.get("contentIndex"),c)}}}return this._isDraggingHandle
},mouseUp:function(b){var a,d,c=this._isDraggingHandle;if(this._isDraggingHandle&&this._mouseDownInfo.didMove){d=Math.max(this._mouseDownInfo.startWidth+b.pageX-this._mouseDownInfo.startPageX,this.get("minWidth"));
this.setPath("content.width",d);a=this.get("displayDelegate");if(a&&a.isTableColumnsDelegate){a.endColumnResizeDrag()
}}this._isDraggingHandle=NO;this._mouseDownInfo=null;return c},mouseEntered:function(){if(!this._isDraggingHandle){this.set("isMouseOver",YES)
}},mouseExited:function(){if(!this._isDraggingHandle){this.set("isMouseOver",NO)}},_isDraggingHandle:NO,_mouseDownInfo:null});
if((typeof SC!=="undefined")&&SC&&SC.bundleDidLoad){SC.bundleDidLoad("sctable")};